...
import com.creek.common.photo.models.ad.AdListener;
...
import com.creek.common.photo.ui.adapter.AlbumItemsAdapter;
import com.creek.common.photo.ui.adapter.PhotosAdapter;
import com.creek.common.photo.ui.dialog.LoadingDialog;
import com.creek.common.photo.ui.widget.PressedTextView;
import com.creek.common.photo.utils.Color.ColorUtils;
import com.creek.common.photo.utils.String.StringUtils;
import com.creek.common.photo.utils.bitmap.BitmapUtils;
import com.creek.common.photo.utils.media.DurationUtils;
import com.creek.common.photo.utils.media.MediaScannerConnectionUtils;
import com.creek.common.photo.utils.permission.PermissionUtil;
import com.creek.common.photo.utils.settings.SettingsUtils;
import com.creek.common.photo.utils.system.SystemUtils;
import com.creek.common.photo.utils.uri.UriUtils;
...
@CreekBean(path = "activity_easy_photos_activity")
public class EasyPhotosActivity extends BaseActivity implements AlbumItemsAdapter.OnClickListener, PhotosAdapter.OnClickListener, AdListener, View.OnClickListener {
    ...
    public static void start(Activity activity, int requestCode) {
        ...
        Launcher.startActivityForResult(activity, EasyPhotosActivity.class, requestCode);
    }
...
//        fragment.startActivityForResult(intent, requestCode);
        ...
        Launcher.startActivityForResult(fragment, EasyPhotosActivity.class, requestCode);
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        loadingDialog = LoadingDialog.get(mContext);
        ...
        initSomeViews();
        if (PermissionUtil.checkAndRequestPermissionsInActivity(mContext, getNeedPermissions())) {
            hasPermissions();
        } else {
    ...
    public void onResume() {
        super.onResume();
        setStatusBarColor(mContext.getResources().getColor(R.color.easy_photos_bar_primary),0);
    }
    ...
    private void hasPermissions() {
        ...
        albumModel.query(mContext, albumModelCallBack);
    }
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull final String[] permissions,
        ...
        PermissionUtil.onPermissionResult(mContext, permissions, grantResults,
                new PermissionUtil.PermissionCallBack() {
                    ...
                    public void onFailed() {
                        ...
                        permissionView.setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View view) {
                                SettingsUtils.startMyApplicationDetailsForResult(mContext,
                                        getPackageName());
                            }
                        });
    ...
    private void toAndroidCamera(int requestCode) {
        ...
        if (cameraIntent.resolveActivity(getPackageManager()) != null ||
            ...
            if (mTempImageFile != null && mTempImageFile.isFile()) {
                ...
                Uri imageUri = UriUtils.getUri(mContext, mTempImageFile);
                ...
                Toast.makeText(mContext, R.string.camera_temp_file_error_easy_photos,
                        Toast.LENGTH_SHORT).show();
            }
        } else {
            Toast.makeText(mContext, R.string.msg_no_camera_easy_photos, Toast.LENGTH_SHORT).show();
        }
    }
...
//        super.onActivityResult(requestCode, resultCode, data);
    ...
    private void addNewPhoto(Photo photo) {
        ...
        if (!isQ) {
            MediaScannerConnectionUtils.refresh(mContext, photo.path);
            folderPath = new File(photo.path).getParentFile().getAbsolutePath();
            albumName = StringUtils.getLastPathSegment(folderPath);
        }

        String albumItem_all_name = albumModel.getAllAlbumName(mContext);
    ...
    private void onCameraResult() {
        LoadingDialog loading = LoadingDialog.get(mContext);
        new Thread(new Runnable() {
            @Override
            public void run() {
                ...
                MediaScannerConnectionUtils.refresh(mContext, mTempImageFile);//
                // 更新媒体库

                Uri uri = UriUtils.getUri(mContext, mTempImageFile);
                int width = 0;
                int height = 0;
                int orientation = 0;
                if (Setting.useWidth) {
    ...
    private void initView() {
        ...
        if (albumModel.getAlbumItems().isEmpty()) {
            if (Setting.isOnlyVideo()) {
                Toast.makeText(mContext, R.string.no_videos_easy_photos, Toast.LENGTH_LONG).show();
                finish();
                return;
            }
            Toast.makeText(mContext, R.string.no_photos_easy_photos, Toast.LENGTH_LONG).show();
        ...
        int columns = mContext.getResources().getInteger(R.integer.photos_columns_easy_photos);
        ...
        photosAdapter = new PhotosAdapter(mContext, photoList, this);

        gridLayoutManager = new GridLayoutManager(mContext, columns);
        if (Setting.hasPhotosAd()) {
    ...
    private void initAlbumItems() {
        ...
        albumItemsAdapter = new AlbumItemsAdapter(mContext, albumItemList, 0, this);
        rvAlbumItems.setLayoutManager(new LinearLayoutManager(mContext));
    ...
    public void onClick(View v) {
        ...
        if (R.id.tv_album_items == id || R.id.iv_album_items == id) {
            ...
            PreviewActivity.start(mContext, -1, 0);
            ...
            processSecondMenu();
            PuzzleSelectorActivity.start(mContext);
        }
    }
...
//        }
    ...
    private void processOriginalMenu() {
        ...
        if (Setting.selectedOriginal) {
            tvOriginal.setTextColor(ContextCompat.getColor(mContext, R.color.easy_photos_fg_accent));
        } else {
            if (Setting.originalMenuUsable) {
                tvOriginal.setTextColor(ContextCompat.getColor(mContext,
    ...
    public void onPhotoClick(int position, int realPosition) {
        PreviewActivity.start(mContext, currAlbumItemIndex, realPosition);
    }
    ...
    public void onSelectorOutOfMax(@Nullable Integer result) {
        if (result == null) {
            if (Setting.isOnlyVideo()) {
                ...
                Toast.makeText(mContext, getString(R.string.selector_reach_max_image_hint_easy_photos,
                        Setting.count), Toast.LENGTH_SHORT).show();
            }
            return;
        }
        switch (result) {
            ...
            case Result.VIDEO_OUT:
                Toast.makeText(mContext, getString(R.string.selector_reach_max_video_hint_easy_photos
                        , Setting.complexVideoCount), Toast.LENGTH_SHORT).show();
                break;
            case Result.SINGLE_TYPE:
                Toast.makeText(mContext, mContext.getResources().getString(R.string.selector_single_type_hint_easy_photos),
                        Toast.LENGTH_SHORT).show();
                break;
